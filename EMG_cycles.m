function [EMG_cycles] = EMG_cycles(data_nan)
...
if ismember('Wedge', data_updated.Properties.VariableNames)
    filtered_data = data_updated(data_updated.SteadyHopping == 1 & data_updated.Wedge_WedgeBack_ == 0 | data_updated.Wedge_WedgeFront_ == 0 | data_updated.Wedge == 0, :);
else
...
EMG_cycles = cell(size(cycles_all));
...
for i = 1:numel(cycles_all)
    ...
    EMG_cycles{i} = current_table(:, selectedColumns);
end

% % gets number of tables in cell array
%  EMG_cycles = numel(cycles);