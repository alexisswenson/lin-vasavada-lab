function [EMG_front_w_cycles] = EMG_front_w_cycles(data_nan)
...
filtered_data = data_updated(data_updated.SteadyHopping == 1 & (data_updated.Wedge_WedgeFront_ == 1 | data_updated.Wedge == 1), :);
else
filtered_data = data_updated(data_updated.SteadyHopping == 1 & (data_updated.Wedge_WedgeFront_ == 1), :);
end
...
EMG_front_w_cycles = cell(size(cycles_all));
...
for i = 1:numel(cycles_all)
    ...
    EMG_front_w_cycles{i} = current_table(:, selectedColumns);
end
